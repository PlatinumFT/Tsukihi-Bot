exports.run = async (client, message, args) => {
    ...
    switch(args[0]) {
        ...
        if (res) {
        await query.qrun(`DELETE FROM guilds where guild_id="${message.guild.id}"`)
        ...
        if(!args[1]) return message.channel.send('Please set a prefix!');
            ...
            if(res) query( `UPDATE guilds SET prefix = "${args[1]}" WHERE guild_id = "${message.guild.id}"`);
            else query(`INSERT INTO guilds values('${message.guild.id}', '${args[1]}')`);

...
module.exports.run = async (bot, message, args, db) => {
    ...
    if(message.guild.id == "204487943568621568") {
        ...
        var numbRoles = rows.length;
            ...
            let roleCount=0;
        if (myRole.id == "336786415407071233" || myRole.id == "206668607327961088" || myRole.id == "271875615064326146" || myRole.id == "322165288332165121" || myRole.id == "220998153044885505" || myRole.id == "338795067097743361") {
        } else {
        for(i=0;i<numbRoles;i++) {
            if (rows[i].role_id == "336786415407071233" || rows[i].role_id == "206668607327961088" || rows[i].role_id == "271875615064326146" || rows[i].role_id == "322165288332165121" || rows[i].role_id == "220998153044885505") {roleCount+=0} else {
